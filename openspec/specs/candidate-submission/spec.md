...
Permettre aux agents RH authentifiés de soumettre un CV texte pour un candidat contre une offre d'emploi, et de déclencher l'analyse en arrière-plan.

## Requirements

### Requirement: RH agent can submit a candidate CV against their own offer
The system SHALL allow an authenticated RH agent to submit a candidate's name and CV text from the offer show page. The submission SHALL create a `candidats` row and an `analyses` row with `statut_analyse = 'pending'`.
...
- **AND** an `analyses` row is created linked to the offer and candidate with `statut_analyse = 'pending'`
...
The system SHALL dispatch a queue job (`AnalyzeCandidateJob`) when a candidate is successfully submitted.

#### Scenario: Job is dispatched on submission
- **WHEN** a candidate is successfully submitted against an offer
- **THEN** an `AnalyzeCandidateJob` is dispatched with the `analyse` ID
