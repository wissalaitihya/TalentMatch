...
Définir le comportement de l'agent IA d'analyse structurée de CV (`CVAnalyzer`) qui extrait des données typées (compétences, expérience, formation, langues, score, recommandation) à partir du texte d'un CV, en utilisant le `HasStructuredOutput` du Laravel AI SDK.
